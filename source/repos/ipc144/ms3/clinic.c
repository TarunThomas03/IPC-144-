...
// To sort the values using bubble sort
void sort(struct Appointment appoints[], int max) {
    ...
    // Convert total minutes to minutes
    for (i = 0; i < max; i++) {
...
void addPatient(struct Patient patient[], int max) {
    ...
    // Checks for the first empty space
    do {
        locate = 0;
        if (patient[i].patientNumber == 0)
        {
...
void removePatient(struct Patient patient[], int max) {
    ...
    // Checks if the patient number exists
    if (findPatientIndexByPatientNum(pNumber, patient, max) != -1) {
        index = findPatientIndexByPatientNum(pNumber, patient, max);
        printf("Name  : %s\n", patient[index].name);
        ...
        do {
            ...
            if ((choice == 'n' || choice == 'N') && (ch == '\n')) {
                ...
                // Places patient to an empty state so other function cannot use it
...
void searchPatientByPatientNumber(const struct Patient patient[], int max) {
    ...
    // Checks if patient number exists
    if (findPatientIndexByPatientNum(pNumber, patient, max) != -1) {
        index = findPatientIndexByPatientNum(pNumber, patient, max);
        printf("Name  : %s\n", patient[index].name);
        printf("Number: %05d\n", patient[index].patientNumber);
...
int findPatientIndexByPatientNum(int patientNumber, const struct Patient patient[], int max) {
    int i;
    // Iterates to find the patient number
    for (i = 0; i < max; i++) {
...
void inputPhoneData(struct Phone* phone) {
    ...
    {
    case 1:
        // Stores the description in the struct
...
int importPatients(const char* datafile, struct Patient patients[], int max) {
    ...
    // Number of records read is returned
    return i;
}
...
int importAppointments(const char* datafile, struct Appointment appoints[], int max) {
    ...
    if (fp != NULL) {
        ...
        // Iterates until file reaches the end of file or the max is reached
        for (i = 0; i < max && !feof(fp); i++) {
            // Reades values from file and stores them
            ...
            // Increments the count if the end of file is not reached
            if (!feof(fp)) {
                count++;
            }
        }
